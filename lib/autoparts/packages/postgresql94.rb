...
module Autoparts
  module Packages
    class PostgreSQL < Package
      ...
      source_url 'http://ftp.postgresql.org/pub/source/v9.4beta2/postgresql-9.4beta2.tar.gz'
      source_sha1 '5b648f84fe3bde23866c701c16a759e0327e52dd'
      source_filetype 'tar.gz'

      def compile
        Dir.chdir('postgresql-9.4beta2') do
          args = [
            '--disable-debug',
            "--prefix=#{prefix_path}",
            "--bindir=#{bin_path}",
      ...
      def tips
        <<-EOS.unindent
          To start the server:
            $ parts start postgresql94

          To stop the server:
            $ parts stop postgresql94
